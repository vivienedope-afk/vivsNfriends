...
if ($token !== '') {
    $app = getApplicationByVerificationToken($conn, $token);
    ...
    if ($app) {
        if (!empty($app['email_verified_at'])) {
            ...
            $ok = markApplicationEmailVerifiedByToken($conn, $token);
            if ($ok) {
                $status = 'verified';
                $message = 'Email verified successfully. Your application is now marked as verified.';
            } else {
                $status = 'invalid';
...
<head>
  ...
  <title>Email Verification - Maia Alta HOA</title>
  <style>
...
<body>
  <div class="wrap">
    <h1 class="title">Maia Alta HOA Email Verification</h1>
